...
router.post('/create', async (req, res) => {
    try {
        ...
        const result = await (0, client_1.query)('INSERT INTO scheduled_jobs (id, workspace_id, name, job_type, cron_expression, config) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *', [id, workspace_id, name, job_type, cron_expression, config ? JSON.stringify(config) : null]);
...
router.get('/list', async (req, res) => {
    try {
        ...
        const result = await (0, client_1.query)(`SELECT j.*, r.started_at as last_run_at, r.status as last_status
...
router.post('/toggle/:jobId', async (req, res) => {
    try {
        const current = await (0, client_1.query)('SELECT * FROM scheduled_jobs WHERE id = $1', [req.params.jobId]);
        if (current.rows.length === 0)
            return res.status(404).json({ error: 'Job not found' });
        const newEnabled = req.body.enabled !== undefined ? !!req.body.enabled : !current.rows[0].enabled;
        const result = await (0, client_1.query)('UPDATE scheduled_jobs SET enabled = $1 WHERE id = $2 RETURNING *', [newEnabled, req.params.jobId]);
        if (newEnabled) {
            try {
                await (0, schedulerService_1.createJob)(result.rows[0]);
            }
            catch (_) { }
        }
        res.json({ job: result.rows[0], success: true });
