        ...
        if (Array.isArray(keywords) && keywords.length > 0) {
            for (const kw of keywords) {
                if (typeof kw === 'string' && kw.trim()) {
                    await (0, client_1.query)('INSERT INTO keywords (id, workspace_id, keyword) VALUES ($1,$2,$3)', [(0, uuid_1.v4)(), id, kw.trim()]);
                }
            }
        }
...
router.post('/switch', async (req, res) => {
    try {
        ...
        const statsRes = await (0, client_1.query)(`SELECT
...
router.get('/:id/keywords', async (req, res) => {
    try {
        const result = await (0, client_1.query)('SELECT * FROM keywords WHERE workspace_id = $1', [req.params.id]);
...
router.post('/:id/keywords', async (req, res) => {
    try {
        ...
        const id = (0, uuid_1.v4)();
        const result = await (0, client_1.query)('INSERT INTO keywords (id, workspace_id, keyword) VALUES ($1,$2,$3) RETURNING *', [id, req.params.id, keyword]);
...
router.delete('/:id/keywords/:kid', async (req, res) => {
    try {
        await (0, client_1.query)('DELETE FROM keywords WHERE id = $1 AND workspace_id = $2', [
            req.params.kid,
            req.params.id,
        ]);
