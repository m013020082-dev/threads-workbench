...
router.post('/search', async (req, res) => {
    try {
        const { workspace_id, keywords, time_range, engagement_threshold, min_followers, max_followers, search_mode, } = req.body;
        // Bug #8：驗證 time_range 為合法值
        const validRanges = ['1h', '6h', '24h', '7d'];
        const safeTimeRange = validRanges.includes(time_range) ? time_range : '24h';
        const { posts, expandedKeywords } = await (0, searchService_1.searchPosts)({
            workspace_id,
            keywords: keywords || [],
        ...
        const ranked = await (0, rankingService_1.rankPosts)(posts, workspace_id, scoringKws, filterStrict);
        for (const r of ranked) {
            await (0, client_1.query)("UPDATE posts SET score = $1, status = 'SCORED' WHERE id = $2", [r.score, r.post.id]);
        }
...
router.post('/score', async (req, res) => {
    try {
        ...
        const postsRes = await (0, client_1.query)("SELECT * FROM posts WHERE workspace_id = $1 AND status = 'DISCOVERED'", [workspace_id]);
        const ranked = await (0, rankingService_1.rankPosts)(postsRes.rows, workspace_id);
        for (const r of ranked) {
            await (0, client_1.query)("UPDATE posts SET score = $1, status = 'SCORED' WHERE id = $2", [r.score, r.post.id]);
        }
        const results = ranked.map(r => ({
...
router.patch('/posts/:id', async (req, res) => {
    try {
        ...
        const result = await (0, client_1.query)('UPDATE posts SET post_text = $1 WHERE id = $2 RETURNING *', [post_text.trim(), id]);
