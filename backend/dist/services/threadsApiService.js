...
exports.publishViaApi = publishViaApi;
...
function extractShortcodeFromUrl(postUrl) {
...
function getOAuthUrl() {
    ...
    if (!appId || !redirectUri)
        throw new Error('缺少 THREADS_APP_ID 或 THREADS_REDIRECT_URI 環境變數');
...
async function exchangeCodeForToken(code) {
    ...
    if (shortData.error)
        throw new Error(shortData.error.message || JSON.stringify(shortData.error));
    const shortToken = shortData.access_token;
    const userId = String(shortData.user_id);
    // Step 2: exchange for long-lived token (60 days)
    const longRes = await fetch(`${LONG_TOKEN_URL}?grant_type=th_exchange_token&client_secret=${appSecret}&access_token=${shortToken}`);
    const longData = await longRes.json();
    if (longData.error)
        throw new Error(longData.error.message || JSON.stringify(longData.error));
...
async function getActiveApiAccount() {
    const res = await (0, client_1.query)('SELECT * FROM publisher_accounts WHERE is_active = true LIMIT 1');
    return res.rows[0] || null;
}
async function saveApiAccount(params) {
    // Deactivate all, then upsert by threads_user_id
    await (0, client_1.query)('UPDATE publisher_accounts SET is_active = false');
    const existing = await (0, client_1.query)('SELECT id FROM publisher_accounts WHERE threads_user_id = $1', [params.userId]);
    if (existing.rows.length > 0) {
        await (0, client_1.query)(`UPDATE publisher_accounts
       SET access_token=$1, token_expires_at=$2, threads_username=$3, is_active=true, updated_at=NOW()
       WHERE threads_user_id=$4`, [params.accessToken, params.expiresAt, params.username, params.userId]);
    }
    else {
        await (0, client_1.query)(`INSERT INTO publisher_accounts
         (id, workspace_id, threads_user_id, threads_username, access_token, token_expires_at, is_active)
       VALUES ($1,$2,$3,$4,$5,$6,true)`, [(0, uuid_1.v4)(), params.workspaceId || null, params.userId, params.username, params.accessToken, params.expiresAt]);
    }
}
// ─── Resolve Threads Media ID via Playwright ─────────────────────────────────
async function resolveMediaIdViaPlaywright(postUrl) {
    ...
    if (mediaId) {
        ...
        // 存入 DB 供下次使用
        await (0, client_1.query)('UPDATE posts SET threads_media_id = $1 WHERE post_url = $2', [mediaId, postUrl]);
    }
    return mediaId;
}
// ─── Reply to Post ────────────────────────────────────────────────────────────
async function replyViaApi(postUrl, replyText) {
    const acc = await getActiveApiAccount();
    if (!acc)
        return { success: false, error: '尚未連接 Threads API，請先在帳號管理中完成 OAuth 授權' };
    // Token expiry check
    ...
    if (!shortcode)
        return { success: false, error: `無法從 URL 解析貼文 ID: ${postUrl}` };
    // 優先順序：1) DB 快取  2) Playwright 攔截（支援無 Cookie 模式）
    let replyToId = null;
    const dbRes = await (0, client_1.query)('SELECT threads_media_id FROM posts WHERE post_url = $1 LIMIT 1', [postUrl]);
...
async function publishViaApi(text) {
    const acc = await getActiveApiAccount();
    if (!acc)
        return { success: false, error: '尚未連接 Threads API，請先完成 OAuth 授權' };
    try {
