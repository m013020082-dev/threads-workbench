...
exports.replyWithCookies = replyWithCookies;
exports.followWithCookies = followWithCookies;
exports.publishWithCookies = publishWithCookies;
...
async function getStorageState() {
    ...
    const acc = await (0, threadsAuth_1.getActiveAccount)();
    if (acc?.session_data) {
        try {
 ...
 */
async function replyWithCookies(postUrl, draftText) {
    let browser = null;
    try {
        const { browser: b, context } = await (0, threadsAuth_1.createAuthenticatedContext)();
        browser = b;
 ...
 */
async function followWithCookies(authorHandle) {
    let browser = null;
    try {
        const { browser: b, context } = await (0, threadsAuth_1.createAuthenticatedContext)();
        browser = b;
        ...
        // 列出頁面按鈕文字診斷
        const allBtnTexts = await page.locator('button, [role="button"]').allTextContents().catch(() => []);
        ...
        for (const sel of followSelectors) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (alreadyFollowedPhrases.some(p => text.includes(p)))
                    continue;
...
async function publishWithCookies(content) {
    const acc = await (0, threadsAuth_1.getActiveAccount)();
    if (!acc?.session_data) {
