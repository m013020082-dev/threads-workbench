...
export function memQuery(text: string, params: unknown[] = []): { rows: Row[] } {
...
function handleInsert(text: string, params: unknown[]): { rows: Row[] } {
...
function handleUpdate(text: string, params: unknown[]): { rows: Row[] } {
  const tableName = text.match(/update\s+(\w+)/i)?.[1]?.toLowerCase() || '';
  if (!tables[tableName]) return { rows: [] };
  ...
  const setPairs: Array<{ col: string; paramIdx: number }> = [];
  if (setMatch) {
    const setStr = setMatch[1];
    const pairs = setStr.split(',').map(s => s.trim());
    for (const pair of pairs) {
      const m = pair.match(/(\w+)\s*=\s*\$(\d+)/);
      if (m) setPairs.push({ col: m[1], paramIdx: parseInt(m[2]) - 1 });
      // Handle SET col = 'literal'
      const lit = pair.match(/(\w+)\s*=\s*'([^']*)'/);
      if (lit) setPairs.push({ col: lit[1], paramIdx: -1 });
    }
  }
  ...
  tables[tableName] = tables[tableName].map(row => {
    ...
    const newRow: Row = { ...row, updated_at: new Date().toISOString() };
    for (const { col, paramIdx } of setPairs) {
      if (paramIdx >= 0) newRow[col] = params[paramIdx];
    }
    if (inlineStatus) newRow['status'] = inlineStatus[1];
    if (inlineApproved) newRow['approved'] = inlineApproved[1] === 'true';
    if (inlineEnabled) newRow['enabled'] = params[parseInt(inlineEnabled[1]) - 1];
    updated.push(newRow);
    return newRow;
  });
...
function handleDelete(text: string, params: unknown[]): { rows: Row[] } {
  ...
  tables[tableName] = tables[tableName].filter(row => {
    if (whereTwoId) return !(row.id === params[0] && row.workspace_id === params[1]);
    if (whereIdMatch) return row.id !== params[parseInt(whereIdMatch[1]) - 1];
    if (whereWsId) return row.workspace_id !== params[parseInt(whereWsId[1]) - 1];
    return false;
  });
...
function handleSelect(text: string, params: unknown[]): { rows: Row[] } {
  ...
  if (whereId) filtered = filtered.filter(r => r.id === params[parseInt(whereId[1]) - 1]);
  if (whereWsId) filtered = filtered.filter(r => r.workspace_id === params[parseInt(whereWsId[1]) - 1]);
  if (wherePostId) filtered = filtered.filter(r => r.post_id === params[parseInt(wherePostId[1]) - 1]);
  if (whereJobId) filtered = filtered.filter(r => r.job_id === params[parseInt(whereJobId[1]) - 1]);
  if (whereEnabledTrue) filtered = filtered.filter(r => r.enabled === true);
  if (statusDiscovered) filtered = filtered.filter(r => r.status === 'DISCOVERED');
  if (statusIn) {
