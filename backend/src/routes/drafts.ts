...
import { generateDrafts } from '../services/draftService';
...
router.post('/generate-drafts', async (req: Request, res: Response) => {
  try {
    ...
    let text = post_text;
    if (!text) {
      const postRes = await query('SELECT * FROM posts WHERE id = $1', [post_id]);
      if (postRes.rows.length === 0) return res.status(404).json({ error: 'Post not found' });
      text = postRes.rows[0].post_text;
    }

    const draftResults = await generateDrafts({
      post_id,
      post_text: text,
    ...
    await query("UPDATE posts SET status = 'DRAFTED' WHERE id = $1", [post_id]);
...
router.get('/drafts/:postId', async (req: Request, res: Response) => {
  try {
...
router.post('/approve-draft', async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      'UPDATE drafts SET approved = true WHERE id = $1 RETURNING *',
      [draft_id]
    );
...
router.delete('/drafts/:draftId', async (req: Request, res: Response) => {
  try {
    await query('DELETE FROM drafts WHERE id = $1', [req.params.draftId]);
