...
function isTaiwanContent(post: Post): boolean {
  if (post.region === 'TW') return true;
...
export function scorePost(post: Post, keywords: string[]): {
  score: number;
  ...
  if (f >= 1000 && f <= 100000) follower_score = 15;
  else if (f >= 100 && f < 1000) follower_score = 8;
  else if (f > 100000) follower_score = 4;

  // Recency bonus (0-10)
  const ageMs = Date.now() - new Date(post.created_at).getTime();
  const ageHours = ageMs / 3600000;
  let recencyScore = 0;
  if (ageHours < 2) recencyScore = 10;
  else if (ageHours < 6) recencyScore = 7;
  else if (ageHours < 24) recencyScore = 4;

  // 台灣地區加分 (0-5): 確認為台灣地區內容優先
  let regionScore = 0;
  if (isTaiwanContent(post)) regionScore += 3;
  if (isTraditionalChinese(post.post_text)) regionScore += 2;
...
export async function rankPosts(
  ...
  const workspaceKeywords = kwRes.rows.map((r: { keyword: string }) => r.keyword);
