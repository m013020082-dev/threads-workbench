...
export async function generateDrafts(request: DraftRequest): Promise<DraftResult[]> {
  // 抓取最近草稿，用於反垃圾郵件檢查
  const historyRes = await query(
    'SELECT draft_text FROM drafts WHERE post_id != $1 ORDER BY created_at DESC LIMIT 20',
    [request.post_id]
  );
  const history: string[] = historyRes.rows.map((r: any) => r.draft_text);
  ...
  // 發文邏輯對應的具體指示
  const logicGuideMap: Record<string, string> = {
    '擬人幽默': `發文邏輯：擬人幽默
- 像真實台灣人在聊天，語氣輕鬆自然
- 可以用一點玩笑或諷刺，但不失禮貌
