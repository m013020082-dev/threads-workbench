...
export function parsePriceCSV(csv: string): PricePoint[] {
  const lines = csv
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter(Boolean);
...
function generateObservations(result: Omit<TechnicalIndicatorResult, 'observations'>): string[] {
  const obs: string[] = [];
  const n = result.count;
  if (n < 2) return ['資料不足，無法產生觀察結論'];
  ...
  if (ma5 !== null && ma20 !== null) {
    if (ma5 > ma20 && lastClose > ma5) obs.push('短期均線在中期均線之上，且收盤價在 MA5 上方 — 短線結構偏多');
    else if (ma5 < ma20 && lastClose < ma5) obs.push('短期均線在中期均線之下，且收盤價在 MA5 下方 — 短線結構偏空');
    else obs.push('均線排列與收盤價位置出現分歧 — 趨勢可能轉折或盤整');
  }
  ...
  if (ma20 !== null && ma60 !== null) {
    ...
    if (prevMa20 !== null && prevMa60 !== null) {
      if (prevMa20 <= prevMa60 && ma20 > ma60) obs.push('MA20 由下穿越 MA60（黃金交叉訊號），但需配合量能與大盤確認');
      if (prevMa20 >= prevMa60 && ma20 < ma60) obs.push('MA20 由上跌破 MA60（死亡交叉訊號），需注意中期趨勢轉弱風險');
    }
  }

  if (rsi !== null) {
    if (rsi > 70) obs.push(`RSI = ${rsi.toFixed(1)}，進入超買區（>70），歷史上常伴隨短線回檔但不必然下跌`);
    else if (rsi < 30) obs.push(`RSI = ${rsi.toFixed(1)}，進入超賣區（<30），歷史上常伴隨短線反彈但不必然上漲`);
    else obs.push(`RSI = ${rsi.toFixed(1)}，落在中性區間（30-70）`);
  }

  if (histLast !== null && histPrev !== null) {
    if (histPrev <= 0 && histLast > 0) obs.push('MACD 柱狀圖剛轉正（多頭動能訊號）');
    else if (histPrev >= 0 && histLast < 0) obs.push('MACD 柱狀圖剛轉負（空頭動能訊號）');
    else if (histLast > 0 && histLast > histPrev) obs.push('MACD 柱狀圖正值持續放大，多頭動能增強中');
    else if (histLast < 0 && histLast < histPrev) obs.push('MACD 柱狀圖負值持續擴大，空頭動能增強中');
  }
...
export function computeIndicators(points: PricePoint[]): TechnicalIndicatorResult {
  if (points.length === 0) {
